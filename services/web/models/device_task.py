...
class DeviceTask:
    ...
    def get_by_id(device_task_id):
        q = """
                select * from device_tasks where id = %(device_task_id)s
            """

        records = Db.execute(q, {'device_task_id': device_task_id}, method='fetch')
        if len(records) == 0:
            raise Exception("No task for device_id '{}' found".format(device_id))

        return records

    @staticmethod
    def get_first_by_device_id(user_identity):
        pass
